...
class cpu:
    ...
    def train_lstm_model(self, model, training_data, epochs, batch_size, lag,__id):
        m_train = training_data.shape[0]
        training_processed = training_data.loc[:__id-1, "cpu"].values
...
def collect_and_store(router_name,username,password, model, __id):
    ...
    cur_cpu = df.iloc[__id]["cpu"]
    
    test_cpu=cpu()
    __id=__id+1    
    if 1:
        cpu_training_complete=df.loc[:__id-1]   
        
        
        print(cpu_training_complete)
        
        model = test_cpu.train_lstm_model(model, cpu_training_complete, 10, 10, lag,__id)
        predict_processed = cpu_training_complete.loc[:__id, "cpu"].values
        ...
        df.iloc[__id+1]["pred_cpu"]=predictions
